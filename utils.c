...
/*   Updated: 2023/07/30 23:44:08 by msander-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void check_args(int argv, char *argc[])
{
	...
	if (ft_isalldigit(argc[1]) != 0)
		exit_error(2, "Invalid number_of_philosophers argument");
	if (ft_isalldigit(argc[2]) != 0)
		exit_error(2, "Invalid time_to_die argument");
	if (ft_isalldigit(argc[3]) != 0)
		exit_error(2, "Invalid time_to_eat argument");
	if (ft_isalldigit(argc[4]) != 0)
		exit_error(2, "Invalid time_to_sleep argument");
	if (argv == 6)
		if (ft_isalldigit(argc[5]) != 0)
			exit_error(2, "Invalid number_of_times_each_philosopher_must_eat argument");
}